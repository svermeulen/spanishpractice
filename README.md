...
- **Scene backgrounds** (optional) — turn on image generation in Settings →
  Visuals to render a photo of each situation behind the chat (with an
  adjustable blur). One image per scenario (not per message) keeps it cheap
  (~$0.008–0.02), and it reuses your OpenAI or Gemini key.
